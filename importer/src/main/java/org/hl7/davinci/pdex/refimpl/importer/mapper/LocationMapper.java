...
class LocationMapper {
    ...
    public LocationMapper(TargetConfiguration targetConfiguration) {
        this.targetConfiguration = targetConfiguration;
    }

    public Location readOrCreate(Reference receivedLocation, ImportRequest importRequest) {
        Location readLocation = importRequest.getReceivedClient()
                .read()
                .resource(Location.class)
                .withId(receivedLocation.getReference())
                .execute();
        return readOrCreate(readLocation, importRequest);
    }

    public Location readOrCreate(Location receivedLocation, ImportRequest importRequest) {
        ...
        if (first.isPresent()) {
            ...
            targetLocation = findByReceivedIdentifier(getReceivedIdentifier(receivedLocation), importRequest);
        }
    ...
    private String getReceivedIdentifier(Location receivedPractitioner) {
